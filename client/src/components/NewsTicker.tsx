...
export default function NewsTicker({ news }: Props) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-[#0a0e1a]/95 border-t border-[#1a2442] backdrop-blur-sm z-40">
      <div className="flex items-stretch overflow-hidden">
        {/* Label */}
        <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-blue-600 text-white">
              ...
              <span key={`${item.id}-${idx}`} className="inline-flex items-center gap-3 px-6">
                <span className="flex items-center gap-1.5">
                  {item.symbols.slice(0, 3).map((s) => (
                    <span key={s} className="mono text-[10px] font-bold text-blue-400 bg-blue-900/40 px-1.5 py-0.5 rounded">
                      {s}
                    </span>
                  ))}
                </span>
