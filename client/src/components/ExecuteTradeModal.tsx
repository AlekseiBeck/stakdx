...
export default function ExecuteTradeModal({ isOpen, onClose, recommendation, currentPrice, onSuccess, session: _session }: Props) {
  ...
  const directionColor = side === 'buy' ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400';
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-sm bg-surface border border-border rounded-2xl shadow-pop"
      ...
      >
        ...
        <div className="px-5 py-4 space-y-4">
          {/* Paper trade warning */}
          <div className="flex items-center gap-2 px-3 py-2 bg-surface-2 border border-border-strong rounded-lg">
            ...
            <p className="text-xs text-muted font-medium">This is a paper trade — no real money involved</p>
          </div>

          {/* Order details */}
          <div className="bg-bg rounded-lg border border-border divide-y divide-border">
            <div className="flex justify-between items-center px-3 py-2">
              <span className="text-xs text-faint">Order Type</span>
              <span className="text-xs mono text-muted font-semibold">Market</span>
            </div>
            <div className="flex justify-between items-center px-3 py-2">
              <span className="text-xs text-faint">Quantity</span>
              <span className="text-xs mono text-fg font-bold">{qty} shares</span>
            </div>
            <div className="flex justify-between items-center px-3 py-2">
              <span className="text-xs text-faint">Current Price</span>
              <span className="text-xs mono text-fg font-bold">${currentPrice.toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center px-3 py-2">
            ...
            <div className="flex justify-between items-center px-3 py-2">
              <span className="text-xs text-faint">Max Risk</span>
              <span className="text-xs mono text-red-700 dark:text-red-400 font-semibold">{maxRisk || '—'}</span>
            </div>
            <div className="flex justify-between items-center px-3 py-2">
              <span className="text-xs text-faint">Stop Loss</span>
              <span className="text-xs mono text-red-700 dark:text-red-400 font-semibold">{stopLoss}</span>
            </div>
          </div>
          ...
          {result === 'success' && (
            <div className="flex items-center gap-2 px-3 py-2.5 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/40 rounded-lg">
              ...
              <p className="text-sm text-emerald-700 dark:text-emerald-400 font-semibold">Order submitted</p>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-1">
            <button
              onClick={handleClose}
              disabled={loading}
              className="flex-1 py-2.5 px-4 rounded-lg border border-border text-muted hover:text-fg hover:border-border-strong text-sm font-semibold transition-all disabled:opacity-50"
            >
              Cancel
            </button>
            <button
